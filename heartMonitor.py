...
class HeartMonitor(object):
	...
	def get_bpm(self):
		...
		fft_data = list(zip((self.bin_to_bpm(b) for b in range(min_bin, max_bin+1)),
...
class Annotator(object):
	...
	def draw_bpm(self, frame, bpm):
		...
		cv2.putText(frame, "%0.0f"%float(bpm), (x,y), self.large_font_outline, .5, c[1])
		cv2.putText(frame, "%0.0f"%float(bpm), (x,y), self.large_font, .5, c[0])
	...
	def draw_fft(self, frame, fft_data, min_bpm, max_bpm):
		w = frame.shape[0]
		h = int(frame.shape[1] * Annotator.FFT_HEIGHT)
		x = 0
		y = frame.shape[1]
		...
		cv2.polylines(frame, numpy.array([line]), False, self.get_colour()[0], 3)
		...
		cv2.putText(frame, text, (x,y), self.small_font_outline, .5, c[1])
		cv2.putText(frame, text, (x,y), self.small_font, .5, c[0])
...
class Program(object):
	...
	def find_face(self, frame):
		...
		if face is not None:
			# Track the new face
			self.face_tracker = FaceTracker(frame, face)


	def sample_frame(self, frame):
		# Get an average of the green channel in on the forehead
		x, y, w, h = self.face_tracker.get_forehead()
		selection = frame[int(x):int(x+w), int(y):int(y+h)]
		#cv2.selectROI(frame, self.face_tracker.get_forehead())
		sample = cv2.mean(selection)[1]
		#print(sample)
